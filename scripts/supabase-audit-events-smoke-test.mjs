...
async function run() {
  ...
  const { recordAuditEvent } = writeService;
  ...
  const createResult = await recordAuditEvent({
    actionType: "smoke.audit_event.inserted",
    entityType: "smoke_check",
    metadata: {
      source: "supabase-audit-events-smoke-test",
  ...
  if (createResult.error || !createResult.data?.id) {
    printResult(
      "WARNING",
      `Teacher: audit insert failed (${createResult.error?.message || "unknown"})`
    );
    failureCount += 1;
  } else {
    createdAuditEventId = createResult.data.id;
    printResult("PASS", `Teacher: audit event inserted (${createdAuditEventId})`);
  }

  if (createdAuditEventId) {
    const teacherRead = await supabase
      .from("audit_events")
      .select("id,actor_profile_id,action_type,entity_type,metadata")
      .eq("id", createdAuditEventId)
      .maybeSingle();
    if (teacherRead.error || !teacherRead.data?.id) {
      printResult(
        "WARNING",
        `Teacher: unable to read own audit event (${teacherRead.error?.message || "unknown"})`
      );
      failureCount += 1;
    } else if (Object.prototype.hasOwnProperty.call(teacherRead.data.metadata || {}, "token")) {
      printResult("WARNING", "Teacher: forbidden token key unexpectedly present in metadata");
      failureCount += 1;
    } else {
      printResult("PASS", "Teacher: own audit event readable and metadata sanitised");
    }
  }
  await signOut();
  ...
  if (!parentSignIn.ok) {
    ...
    if (parentRead.error || !parentRead.data) {
      printResult("PASS", "Parent: audit event read blocked as expected");
    } else {
  ...
  if (!hqSignIn.ok) {
    ...
    const hqRead = await supabase
      .from("audit_events")
      .select("id,action_type,entity_type")
      .eq("id", createdAuditEventId)
      .maybeSingle();
    if (hqRead.error || !hqRead.data?.id) {
      ...
      printResult("PASS", "HQ Admin: audit event visible");
    }
    ...
    if (cleanup.error || !cleanup.data?.id) {
      ...
      printResult("PASS", "HQ Admin: cleanup deleted smoke audit event");
    }
  }
  await signOut();

  if (failureCount > 0) {
    printResult("WARNING", `Audit-events smoke finished with ${failureCount} failing checks`);
    process.exit(1);
  }
  if (warningCount > 0) {
    printResult("CHECK", `Audit-events smoke finished with ${warningCount} warning/check items`);
  }
}
