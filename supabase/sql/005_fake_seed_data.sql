...
-- Before running this seed file, create the 5 fake Auth users in Supabase Authentication:
...
--
-- This seed resolves profile IDs from auth.users by fake email.
-- If any fake auth user is missing, this file should fail early.
...
begin
  ...
  if missing_count > 0 then
    raise exception
      'Missing % required fake auth users. Create all 5 fake users in Authentication -> Users before running 005.',
      missing_count;
  end if;
end
$$;
...
insert into profiles (id, email, full_name, role, branch_id, linked_student_id) values
  ((select id from auth.users where email = 'hq.demo@example.test' limit 1), 'hq.demo@example.test', 'HQ Demo User', 'hq_admin', null, null),
  ((select id from auth.users where email = 'supervisor.demo@example.test' limit 1), 'supervisor.demo@example.test', 'Supervisor Demo User', 'branch_supervisor', '11111111-1111-1111-1111-111111111111', null),
  ((select id from auth.users where email = 'teacher.demo@example.test' limit 1), 'teacher.demo@example.test', 'Teacher Demo User', 'teacher', '11111111-1111-1111-1111-111111111111', null),
  ((select id from auth.users where email = 'parent.demo@example.test' limit 1), 'parent.demo@example.test', 'Parent Demo User', 'parent', '11111111-1111-1111-1111-111111111111', null),
  ((select id from auth.users where email = 'student.demo@example.test' limit 1), 'student.demo@example.test', 'Student Demo User', 'student', '11111111-1111-1111-1111-111111111111', '55555555-5555-5555-5555-555555555555')
on conflict (id) do nothing;
...
insert into teachers (id, profile_id, branch_id) values
  ('44444444-4444-4444-4444-444444444444', (select id from auth.users where email = 'teacher.demo@example.test' limit 1), '11111111-1111-1111-1111-111111111111')
on conflict (id) do nothing;
...
insert into guardians (id, profile_id) values
  ('66666666-6666-6666-6666-666666666666', (select id from auth.users where email = 'parent.demo@example.test' limit 1))
on conflict (id) do nothing;
...
insert into homework_attachments (id, branch_id, class_id, student_id, homework_record_id, uploaded_by_profile_id, storage_bucket, storage_path, file_name) values
  ('21212121-2121-2121-2121-212121212121', '11111111-1111-1111-1111-111111111111', '33333333-3333-3333-3333-333333333331', '55555555-5555-5555-5555-555555555555', '20202020-2020-2020-2020-202020202020', (select id from auth.users where email = 'parent.demo@example.test' limit 1), 'homework-uploads', 'demo/student-001/homework-001.txt', 'homework-demo.txt')
on conflict (id) do nothing;
...
insert into teacher_tasks (id, branch_id, class_id, student_id, created_by_profile_id, title, details, status, due_at) values
  ('40404040-4040-4040-4040-404040404040', '11111111-1111-1111-1111-111111111111', '33333333-3333-3333-3333-333333333331', '55555555-5555-5555-5555-555555555555', (select id from auth.users where email = 'supervisor.demo@example.test' limit 1), 'Demo follow-up task', 'Prepare demo revision plan', 'in_progress', now() + interval '2 days')
on conflict (id) do nothing;

insert into teacher_task_assignments (id, task_id, teacher_id, assigned_by_profile_id, status) values
  ('41414141-4141-4141-4141-414141414141', '40404040-4040-4040-4040-404040404040', '44444444-4444-4444-4444-444444444444', (select id from auth.users where email = 'supervisor.demo@example.test' limit 1), 'in_progress')
on conflict (id) do nothing;

insert into task_attachments (id, task_id, uploaded_by_profile_id, storage_bucket, storage_path, file_name) values
  ('42424242-4242-4242-4242-424242424242', '40404040-4040-4040-4040-404040404040', (select id from auth.users where email = 'teacher.demo@example.test' limit 1), 'task-attachments', 'demo/tasks/task-001.txt', 'task-demo-note.txt')
on conflict (id) do nothing;
...
insert into observations (id, branch_id, class_id, teacher_id, observer_profile_id, observation_note) values
  ('60606060-6060-6060-6060-606060606060', '11111111-1111-1111-1111-111111111111', '33333333-3333-3333-3333-333333333331', '44444444-4444-4444-4444-444444444444', (select id from auth.users where email = 'supervisor.demo@example.test' limit 1), 'Demo observation note')
on conflict (id) do nothing;
