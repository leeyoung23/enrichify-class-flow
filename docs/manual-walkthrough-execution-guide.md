...
2. Open **Source Evidence Preview** — confirm attendance/homework/parent-comms/curriculum/memories behave as expected for your tenant; confirm **Released homework feedback** reflects released-to-parent rows only (or warm empty state); confirm the **Learning context · teacher notes · goals** card shows sanitised roll-up from **student_school_profiles + learning_goals** (not the separate classroom Observations module) when your **report period overlaps seeded goal/profile timestamps** — or warm empty copy, labelled **staff-only / not auto-sent to parents**; optional dev check: `npm run test:supabase:ai-parent-report:observation-evidence` after `013` fake seed. Confirm **worksheet OCR** remains a deferred placeholder (not silent automation).
