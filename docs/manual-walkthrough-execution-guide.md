...
2. Open **Source Evidence Preview** — confirm attendance/homework/parent-comms/curriculum/memories behave as expected for your tenant; confirm **Released homework feedback** reflects released-to-parent rows only (or warm empty state); confirm **Teacher observations (staff evidence)** shows sanitised roll-up from **school profile notes + scoped learning goals** when data exists (or warm empty copy), labelled **staff-only / not auto-sent to parents**; confirm **worksheet OCR** remains a deferred placeholder (not silent automation).
