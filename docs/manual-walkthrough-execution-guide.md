...
**Evidence pipeline reference:** `docs/ai-evidence-pipeline-readiness-plan.md` (what aggregates today vs placeholders). **Future structured Monthly Learning Observation:** `docs/guided-teacher-observation-tasks-plan.md` (rubric + naming), and `docs/monthly-learning-observation-schema-rLS-plan.md` (schema/RLS/task typing design). Backend foundation SQL: `supabase/sql/045_student_learning_observations_foundation.sql` (manual apply; not wired into AI aggregation yet).
