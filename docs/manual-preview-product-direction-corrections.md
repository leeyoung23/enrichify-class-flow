...
| **AI Parent Reports** | Manual and mock fields are **temporary source notes** or overrides. Long term, drafts should use **system evidence** (attendance, homework, lesson progression, worksheet/homework uploads, observations, memories/media, teacher-approved notes) with **teacher review/edit**—not empty-page authoring. **No parent release** without explicit staff release. |
...
- `src/pages/AiParentReports.jsx` — source-alignment copy; **Report source data preview** card (demo/fake labels only; no new data fetches)
